...
public class cipher {
	public static void main(String[] args) {
		...
		if(aJNI.ktb_cipher_init(ctx,
					...
					ArcanaJNI.ktb_cipher_mode_t.KTB_CIPHER_MODE_CBC, true) ==
		...
		try {
			...
			do {
				...
				if(read > 0) {
					...
					if(pad_output_size) {
						blockrounded_read_count = (read / block_size) * block_size;

						if(read % block_size != 0) {
							blockrounded_read_count += block_size;
						}
                  System.out.println("blockrounded_read_count : " + blockrounded_read_count);
					}
					else {
               ...
               System.arraycopy(data, 0, buffer_out, 0, buffer_out.length);
					...
					if(aJNI.ktb_cipher_encrypt_block(ctx.getAddress(), buffer_in, buffer_out) ==
							ArcanaJNI.ktb_errno.KTB_ERR_NO_ERROR) {
			...
			long suffix_size = aJNI.ktb_cipher_get_suffix_size(ctx.getAddress());
			...
			if(aJNI.ktb_cipher_init(ctx,
						ArcanaJNI.ktb_cipher_algo_t.KTB_CIPHER_ALGO_AES256,
						ArcanaJNI.ktb_cipher_mode_t.KTB_CIPHER_MODE_CBC, false) ==
					ArcanaJNI.ktb_errno.KTB_ERR_NO_ERROR) {
		...
		if(aJNI.ktb_cipher_set_key(ctx.getAddress(), key) == ArcanaJNI.ktb_errno.KTB_ERR_NO_ERROR) {
         ...
         System.out.println("buffer_size : " + buffer_size);
			long prefix_size = aJNI.ktb_cipher_get_prefix_size(ctx.getAddress());
			...
			do {
               ...
               System.arraycopy(data2, 0, buffer_out, 0, buffer_out.length);
					...
					if(aJNI.ktb_cipher_decrypt_block(ctx.getAddress(), buffer_out, buffer_out) ==
							ArcanaJNI.ktb_errno.KTB_ERR_NO_ERROR) {
